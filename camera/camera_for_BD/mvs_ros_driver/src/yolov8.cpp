...
std::vector<std::vector<cv::cuda::GpuMat>> YoloV8::preprocess(const cv::cuda::GpuMat &gpuImg) {
    ...
    if (resized.rows != inputDims[0].d[1] || resized.cols != inputDims[0].d[2]) {
        ...
        resized = Engine<float>::resizeKeepAspectRatioPadRightBottom(gpuImg, inputDims[0].d[1], inputDims[0].d[2]);
    }
...
void YoloV8::blurDetectedObjectsOnGPU(cv::cuda::GpuMat &d_image, const std::vector<Object> &objects) {
    for (const auto &obj : objects) {
        // Define the Region of Interest (ROI) as the object's bounding box, clamped to the image size
        cv::Rect roi = obj.rect;

        if (roi.area() == 0) continue;

        // Extract ROI from the GPU image
        cv::cuda::GpuMat d_roi = d_image(roi);

        // Create a Gaussian filter (ddepth = sdepth = CV_8U for 8-bit images)
        cv::Ptr<cv::cuda::Filter> gaussianFilter = cv::cuda::createGaussianFilter(
            d_roi.type(),       // Input type (e.g., CV_8UC3)
            CV_8UC3,                // output type (often same as input)
            cv::Size(31, 31),   // Kernel size
            0,                  // Sigma X (0 = auto-calculated)
            0,                  // Sigma Y (0 = same as Sigma X)
            cv::BORDER_DEFAULT
        );
        ...
        // Apply three times for stronger blur
        gaussianFilter->apply(d_roi, d_blurred);
        d_blurred.copyTo(d_roi);
        gaussianFilter->apply(d_roi, d_blurred);
        d_blurred.copyTo(d_roi);
        gaussianFilter->apply(d_roi, d_blurred);
        d_blurred.copyTo(d_roi);
